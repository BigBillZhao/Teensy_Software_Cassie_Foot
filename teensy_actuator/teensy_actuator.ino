...
void loop() {
  ...
  Serial.println((static_cast<uint16_t>(retract[3]) << 8) | static_cast<uint16_t>(retract[2]));
  // sets the target position and modifies the register 0x29 to 0 steps (fully retracted)
  sendCommand(retract);
  ...
  // Sets position to 1000 steps (Register address (0x29, 0x00), 1000 steps (0x03E8 split as shown below))
  uint8_t halfway[] = {0x29, 0x00, 0xE8, 0x03};

  Serial.print("Moving to position: ");
  Serial.println((static_cast<uint16_t>(halfway[3]) << 8) | static_cast<uint16_t>(halfway[2]));
  ...
  Serial.print("Moving to position: ");
  Serial.println((static_cast<uint16_t>(fullextend[3]) << 8) | static_cast<uint16_t>(fullextend[2]));
  // sets the target position and modifies the register 0x29 to 1000 steps (halfway extended)
  ...
  Serial.print("Moving to position: ");
  Serial.println((static_cast<uint16_t>(halfway2[3]) << 8) | static_cast<uint16_t>(halfway2[2]));
  // // sets the target position and modifies the register 0x29 to 1000 steps (halfway extended)
